#!/usr/bin/env python3
"""Build app/sponsors.js: seed sponsors + per-route nearby locations (PRD-revenue.md).
...
def main():
    routes = json.loads((ROOT / "app" / "routes.js").read_text().split("const ROUTES = ", 1)[1].rstrip().rstrip(";"))
    ...
    (ROOT / "app" / "sponsors.js").write_text(js, encoding="utf-8")
    n_routes = len(out["by_route"])
    n_pins = sum(len(v) for r in out["by_route"].values() for v in r.values())
    print(f"app/sponsors.js: {len(out['sponsors'])} sponsor(s), pins on {n_routes} routes ({n_pins} route-pin pairs)")
