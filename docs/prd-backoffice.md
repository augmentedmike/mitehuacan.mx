...
- **The current `/system/*` pages become the seed of the separate app.** They
  ...
  mechanical: move them to `admin/` (own mini-project, own deploy), point
...
- **Own repo:** `github.com/augmentedmike/mitehuacan-admin` (private). The
  `admin/` tree graduates out of the main repo and becomes the repo root.
...
```
/goal Build the route recorder module per docs/prd-backoffice.md §10.1: migration
0012 (ride_sessions, stop_events) applied local+staging, POST/GET /api/track in
admin/functions/api/track.js (STATS_TOKEN auth, positions rows with
...
/goal Build the sponsors module per docs/prd-backoffice.md §4 (single STATS_TOKEN
